...
vector<GLfloat> planeColor(GLfloat R, GLfloat G, GLfloat B) {
    vector<GLfloat> normlizedColor = { R / 256.0f, G / 256.0f, B / 256.0f };
    return composeItems({ normlizedColor, normlizedColor, normlizedColor, normlizedColor });
}

vector<GLfloat> cubeColor(GLfloat R, GLfloat G, GLfloat B) {
    return composeItems({ planeColor(R, G, B), planeColor(R, G, B), planeColor(R, G, B), planeColor(R, G, B) , planeColor(R, G, B) , planeColor(R, G, B) });
}
...
vector<GLfloat> color_table() {
    //TableTop
    vector<GLfloat> tabletopCenter = cubeColor(225,183,67);
    vector<GLfloat> tabletopRim1 = cubeColor(189, 143, 37);
    vector<GLfloat> tabletopRim2 = cubeColor(185, 148, 65);
    vector<GLfloat> tabletopRim3 = cubeColor(206, 153, 34);
    vector<GLfloat> tabletopRim4 = cubeColor(206, 171, 34);
    vector<GLfloat> tabletop = composeItems({ tabletopCenter , tabletopRim1 , tabletopRim2 ,tabletopRim3 ,tabletopRim4 });
    //Legs
    vector<GLfloat> tableg0 = cubeColor(165, 129, 46);
    return composeItems({ tabletop, tableg0, tableg0, tableg0, tableg0 });
}
...
vector<GLfloat> color_couch() {
    //Structure
    vector<GLfloat> bottom = cubeColor(0,0,10);
    //Front, Back, Right, Left, Up, Down
    vector<GLfloat> side = composeItems({planeColor(10,10,80), planeColor(0,0,40) , planeColor(0,0,40) , planeColor(0,0,10) , planeColor(0,0,100) , planeColor(0,0,0) });
    vector<GLfloat> back = cubeColor(0, 0, 20);
    //Cushions
    vector<GLfloat> bottomCushion = composeItems({ planeColor(10,10,80), planeColor(0,0,40) , planeColor(0,0,40) , planeColor(0,0,10) , planeColor(50,50,150) , planeColor(0,0,0) });
    vector<GLfloat> backCushion = composeItems({ {1.0,1.0,1.0,10.0 / 256.0,10.0 / 256.0,80.0 / 256.0,10.0 / 256.0,10.0 / 256.0,80.0 / 256.0,10.0 / 256.0,10.0 / 256.0,80.0 / 256.0}, planeColor(0,0,40) , planeColor(0,0,40) , planeColor(0,0,10) , planeColor(50,50,150) , planeColor(0,0,0) });
    vector<GLfloat> cushions0 = composeItems({ bottomCushion, backCushion });
    //Assemble Main Sections
    vector<GLfloat> couchSection0 = composeItems({ bottom, side, back, cushions0, cushions0 });
    //Corner Section
    vector<GLfloat> cornerBottom = cubeColor(0, 0, 0);
    vector<GLfloat> couchCorner = composeItems({ cornerBottom, cushions0, backCushion });
    return composeItems({ couchSection0, couchSection0, couchCorner });
}



vector<GLfloat> color_lamp() {
    vector<GLfloat> base = cubeColor(242,138,75);
    vector<GLfloat> shadeBase = cubeColor(190, 26, 26);
    vector<vector<GLfloat>> lampShadeSections;
    float iterations = 10.0f;
    for (int i = 0; i < iterations; i++) {
        lampShadeSections.push_back(shadeBase);
    }
...
vector<GLfloat> color_candle() {
    vector<GLfloat> holder = cubeColor(200, 181, 180);
    vector<GLfloat> candle = cubeColor(222,242,75);
...
vector<GLfloat> color_pillow() {
    vector<GLfloat> base = cubeColor(110, 0, 0);
    vector<vector<GLfloat>> frontSections, backSections;
    int iterations = 9;
    for (int i = 0; i < iterations; i++) {
        frontSections.push_back(cubeColor(120 + i*(10), i*2, i * 2));
    }
    for (int i = 0; i < iterations; i++) {
        backSections.push_back(cubeColor(120 - i * (15), i * 2, i * 2));
    }
    vector<GLfloat> front = composeItems(frontSections);
    vector<GLfloat> back = composeItems(backSections);
...
vector<GLfloat> color_ottoman() {
    vector<GLfloat> bottomSection = composeItems({ planeColor(185, 58, 52), planeColor(198, 95, 91), planeColor(224, 67, 59), 
                                                    planeColor(205, 38, 30), planeColor(247, 128, 118), planeColor(0, 0, 0) });
    vector<GLfloat> topSection = composeItems({ planeColor(205, 38, 30), planeColor(247, 128, 118),
                                                    planeColor(185, 58, 52), planeColor(198, 95, 91), planeColor(224, 67, 59),planeColor(0, 0, 0)
                                                });
...
vector<GLfloat> init_scene() {
    ...
    vector<GLfloat> pillow3 = mat_mult({ translation_matrix(-7.8,3,-7.8), rotation_matrix_y(50), rotation_matrix_x(-10), scaling_matrix(2.7, 2.7, 2.7) }, basicPillow);
...
vector<GLfloat> init_color(vector<GLfloat> scene) {
    ...
    colors = composeItems({ colors, color_couch(), color_table(), color_table(), color_ottoman(), color_pillow(), color_pillow(), color_pillow(), color_lamp(), color_candle() });
    /*
    for (int i = 0; i < scene.size()/(3*4*6); i++) {
        colors = composeItems({ colors, cubeColor(150,0,0), cubeColor(0,150,0), cubeColor(0,0,150),cubeColor(256,0,0), cubeColor(0,256,0),cubeColor(0,0,256) });
    }
    */
    /*
    for (int i = 0; i < scene.size()/3 - 4; i++) {
        float planeColorR = 0.25f + ((static_cast<float>(rand() % 1000) / static_cast<float>(1000))/2.0f);
        float planeColorG = 0.0f + ((static_cast<float>(rand() % 1000) / static_cast<float>(1000)) / 2.0f);
        float planeColorB = 0.5f + ((static_cast<float>(rand() % 1000) / static_cast<float>(1000)) / 2.0f);
        colors = composeItems({ colors,  {planeColorR, planeColorG, planeColorB}, {planeColorR, planeColorG, planeColorB} , {planeColorR, planeColorG, planeColorB} , {planeColorR, planeColorG, planeColorB} });
    }
    */
    /*
